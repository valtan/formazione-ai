...
FD="/sessions/inspiring-sleepy-brahmagupta/mnt/.claude/skills/canvas-design/canvas-fonts"
LOGO="/sessions/inspiring-sleepy-brahmagupta/mnt/formazione-ai/context/logo_piamarta_formazione.png"
OUT="/sessions/inspiring-sleepy-brahmagupta/mnt/outputs/brochure_v29.pdf"
...
for cx in cols:
    line(cx,570,cx+93.3,570,COBALT,2.0)
...
mono(48,249.9,"COSA RESTA IN AZIENDA","GMB",10.0,COBALT)
...
mono(48,537.9,"IL PERCORSO — DALL'ASSESSMENT AI RISULTATI","GMB",10.0,COBALT)
line(54,571.9,428.5,571.9,LINEP2,1.0)
...
for cx,lab,lines_,ncol in nodes:
    ...
    circ(cx,571.9,6.0,fill=PAPER,stroke=ncol,w=2.2)
    circ(cx,571.9,2.2,fill=inner)
    mono(cx,593.9,lab,"GMB",7.5,COBALT,track=1.4)
    for i,ln in enumerate(lines_):
        txt(cx,605.9+i*12,ln,"ISB",10.0,INK)
# follow-up
rect(54,626.4,58.5,630.9,ACID)
txt(66,634.9,"Follow-up:","ISB",10.0,INK)
fw=pdfmetrics.stringWidth("Follow-up:","ISB",10.0)
txt(66+fw+3,634.9," consulenza continuativa opzionale dopo il corso, per accompagnarvi nell'evoluzione dei processi.".strip(),"IS",10.0,GREY)
...
line(48,728,400,728,LINEDK,0.7)

# contatti (colonna sinistra) — 4 blocchi compatti + riga accreditamento
mono(48,748,"PIAMARTA LAVORO E SVILUPPO","GMB",8.0,ACID,track=1.0)
mono(48,764,"A.F.G.P. ASSOCIAZIONE FORMAZIONE GIOVANNI PIAMARTA","GM",6.5,MIST,track=0.7)
txt(48,786,"Via Enrico Ferri 73, Brescia — Tel. 030 6481405","IS",9.0,BODYDK)
txt(48,804,"piamartalavoroesviluppo@piamarta.it — www.afgp.it/ples.html","IS",9.0,BODYDK)
txt(48,822,"Ente accreditato Regione Lombardia per i Servizi di Formazione","IS",8.0,MIST)

# QR code — riservato l'INTERO spazio a destra del footer (colonna piena, box Paper arrotondato)
QR="/sessions/inspiring-sleepy-brahmagupta/mnt/formazione-ai/comunicazione/stampa/qr-afgp-ples.png"
qx1=547.3; qbox=130; qx0=qx1-qbox; qtop=688; qbot=qtop+qbox; qpad=10
c.setFillColorRGB(*WHITE)
c.roundRect(qx0,Y(qbot),qbox,qbox,10,stroke=0,fill=1)
c.drawImage(ImageReader(QR),qx0+qpad,Y(qbot-qpad),width=qbox-2*qpad,height=qbox-2*qpad,mask='auto')
mono(qx1,830,"Clicca o inquadra il QR code","GM",6.8,MIST,align="r",track=0.4)
